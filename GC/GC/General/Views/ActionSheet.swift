...
class ActionSheet: UIView {
    ...
    private init(title: String? = nil, items: [String], cancel: String? = nil, selectedIndex: ((Int) -> Void)?) {
        ...
        // 标题和按钮背景视图
        bottomView                  = UIView(frame: CGRect(x: 0, y: DEVICE_SCREEN_HEIGHT, width: DEVICE_SCREEN_WIDTH, height: bottomViewH))
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.bottomView.frame.origin.y = DEVICE_SCREEN_HEIGHT - bottomViewH
        }
    }
    ...
    @objc fileprivate func tapGesture() {
        UIView.animate(withDuration: 0.3, animations: { 
            self.backgroundColor = UIColor.clear
            self.bottomView.frame.origin.y = DEVICE_SCREEN_HEIGHT
        }) { (finished) in
